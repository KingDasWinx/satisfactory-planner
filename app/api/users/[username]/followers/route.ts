...
export async function GET(req: Request, { params }: Params) {
  ...
  const rl = rateLimit(`users:followers:${username}:${ip}`, { windowMs: 60_000, limit: 600 })
  ...
  const followers = await prisma.follow.findMany({
    where: {
      followingId: user.id,
      ...(q ? { follower: { OR: [{ username: { contains: q } }, { name: { contains: q, mode: 'insensitive' } }] } } : {}),
    },
    orderBy: { createdAt: 'desc' },
    select: {
      follower: { select: { username: true, name: true, image: true } },
    },
    take: 200,
  })
