...
export function EditProfileForm({ initialBio = '', initialIsPrivate = false, initialLinks = {} }: EditProfileFormProps) {
  ...
  async function save() {
    ...
    setLoading(false)
    if (!res) { setMsg('Falha de rede.'); return }
    if (res.status === 401) { setMsg('Faça login para editar seu perfil.'); return }
    ...
    <div className="rounded-2xl border border-slate-800 bg-slate-900 px-5 py-4 space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[11px] text-slate-500">Se ativado, visitantes veem só avatar + @username.</p>
        </div>
        <button
          type="button"
